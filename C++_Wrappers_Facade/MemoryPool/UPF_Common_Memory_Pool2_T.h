...
/*********************************************************************************************************************
*       FILE NAME       : UPF_Common_Memory_Pool2_T.h                                                                *
*       CREATE DATE     : 2007-11-12                                                                                 *
*       MODULE          : Memory pool                                                                                *
*       AUTHOR          : Gao Yanbing                                                                                *
*--------------------------------------------------------------------------------------------------------------------*
*       MEMO            : [CN]可变对象大小的内存池![CN]                                                              *
*********************************************************************************************************************/
#ifndef UPF_COMMON_MEMORY_POOL2_T_H
#define UPF_COMMON_MEMORY_POOL2_T_H

#include "UPF_Memory_Pool_Base_T.h"     /* for base class of UPF_Common_Memory_Pool2 */
...
/** 
 * @class UPF_Common_Memory_Pool2_T
 * 
...
class UPF_Common_Memory_Pool2_T : public UPF_Memory_Pool_Base< Allocator,
                                                             Lock,
                                                             RecordPolicy >
{
public:
    /// 初始化内部状态.
    UPF_Common_Memory_Pool2_T( const char *          memory_pool_name,
                               size_t                allocated_memory_limit = MAX_MEMORY_REQ_SIZE,
                               toplevel_allocator_t  allocator_type = UPF_TOPLEVEL_PERMANENT_ALLOCATOR );

    /// 清除内存池内部申请的所有内存
    ~UPF_Common_Memory_Pool2_T( void );
...
private:
    ...
    /// 大内存块的节点类型 (memory size > 16K), Large_Block_Node的第一个字段
    ...
    /// 当每次分配内存时, 如果没有可用内存, 需要从系统分配出一大块内存, 一般为
...
private:
    ...
    void * refill( size_t size, int free_index );

    /// 从系统中申请更多的内存块.
    void * allocate_blocks( size_t size, int & num_objs );
...
private:
    /// 用于存放free small block, 具体请参见UPF_Common_Memory_Pool2的说明.
...
#include "UPF_Common_Memory_Pool2_T.cpp"
#endif /* UPF_TEMPLATES_REQUIRE_SOURCE */

#endif /* UPF_COMMON_MEMORY_POOL2_T_H */
