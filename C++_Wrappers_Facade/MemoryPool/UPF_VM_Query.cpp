...
/*********************************************************************************************************************
*       FILE NAME       : UPF_VM_Query.cpp                                                                           *
*       CREATE DATE     : 2008-6-26                                                                                  *
*       MODULE          : Memory Pool                                                                                *
*       AUTHOR          : Gao Yanbing                                                                                *
*--------------------------------------------------------------------------------------------------------------------*
*       MEMO            : [CN]²éÑ¯ÐéÄâÄÚ´æÒ³µÄ×´Ì¬![CN]                                                            *
*********************************************************************************************************************/
             ...
             VMQUERY_HELP * pVMQHelp ) 
{
   ...
   BOOL fOk = (VirtualQuery(pvAddress, &mbi, sizeof(mbi)) 
      == sizeof(mbi));
...
BOOL 
VMQuery( LPCVOID pvAddress, PVMQUERY pVMQ ) 
{
...
PCSTR 
GetMemStorageText( DWORD dwStorage ) 
{
...
PSTR 
GetProtectText( DWORD dwProtect, PSTR szBuf, BOOL fShowFlags ) 
{
   ...
   if (fShowFlags) {
      strcat(szBuf, " ");
      strcat(szBuf, (dwProtect & PAGE_GUARD)        ? "G" : "-");
      strcat(szBuf, (dwProtect & PAGE_NOCACHE)      ? "N" : "-");
      strcat(szBuf, (dwProtect & PAGE_WRITECOMBINE) ? "W" : "-");
   }
...
void 
ConstructRgnInfoLine( PVMQUERY pVMQ, PSTR szLine, int nMaxLen )
{
    sprintf(szLine, "%p     %s  %16u  ",
            pVMQ->pvRgnBaseAddress,
            GetMemStorageText(pVMQ->dwRgnStorage),
            pVMQ->RgnSize );
    
    if (pVMQ->dwRgnStorage != MEM_FREE) {
        sprintf( strchr(szLine, 0), "%5u      ", pVMQ->dwRgnBlocks );
...
void 
ConstructBlkInfoLine( PVMQUERY pVMQ, PSTR szLine, int nMaxLen )
{
   sprintf( szLine, "   %p  %s  %16u             ",
   ...
   {
      GetProtectText(pVMQ->dwBlkProtection, strchr(szLine, 0), TRUE);
   }
}
