...
class helpFunc:
    ...
    def getlogfiles(path, isMxlog = False):
        try:
            log_files= []
...
class LogParser:
    ...
    def enableParseMxlog(self):
        self.bParseMxlog = True

    def parseLogFiles(self, pathname):
        logfiles = []
        if(len(pathname) == 0):
            logfiles = helpFunc.getlogfiles("", self.bParseMxlog)
        elif os.path.isdir(pathname):
            logfiles = helpFunc.getlogfiles(pathname, self.bParseMxlog)
        elif os.path.isfile(pathname):
            if(self.bParseMxlog):
                return self.parseMXlog(pathname)
            else:
                return self.parseLog(pathname)

        for filename in logfiles:
            if (self.bParseMxlog):
                self.parseMXlog(pathname)
            else:
                self.parseLog(pathname)
    ...
    def parseLog(self, logfileName):
        ...
        try:
            ...
            for line in open(logfileName):
                ...
                if(pos != -1):
                    self.parseGyroline(line, tempArry, dataArry)
